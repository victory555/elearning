...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_SESSION["type_admin"] == 1) {
  if (isset($_POST["categorieId"]) && !empty($_POST["categorieId"])) {
    $id_categorie = check_input($_POST["categorieId"]);
    $ok = true;
  } else {
  ...
  if ($ok) {
    $requete = $connect->prepare("DELETE FROM categories WHERE id_categorie=?");
    $requete->execute(array($id_categorie));
    $_GET["id_categorie"] = $id_categorie;
...
<body class="g-sidenav-show bg-gray-100">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark font-weight-bolder" aria-current="page">categories</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">Les categories <?php if (isset($id_admin)) echo $id_admin; ?> </h6>
        </nav>
              ...
              <div class="card-body px-0 pt-0 pb-2">
                <div class="table-responsive p-0">
                  <table class="table align-items-center mb-0 table-striped table-bordered table-hover" id="myTable">
                    <thead>
                      <tr class="no-print">
                        <td class="text-sm"><span class="fas fa-list mx-3"></span>Nombre de categories : <span class="bg-dark p-2 rounded-pill text-white text-b" style="font-weight:bold;"><?php echo count($categories); ?></span></td>
                        <td class="text-sm" colspan="2">Ajouter une categorie
                          <a href="add_categorie.php" class="btn btn-primary btn-sm mx-3 rounded-pill <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>">
                            <span class="fas fa-plus"></span>
                          </a>
                        </td>
                      </tr>
                      <tr>
                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">categories <a href="#" id="sortName"><span class="fas fa-sort mx-3" data-toggle="tooltip" data-placement="bottom" title="Trier les élements"></span></a></th>
                          ...
                          <td>
                              <div class="d-flex flex-column justify-content-center">
                                  <div style="align-items:center;">
                                    <span class="btn btn-sm btn-dark rounded-pill my-2" style="border-right: 5px solid yellow; padding-right: 10px;"><?= $categorie['id_categorie'] ?></span>
                                    <h6 class="mb-0 text-sm text-bold" style="display: inline-block;"><?= $categorie['nom_categorie'] ?></h6>

                                  </div>                                
                          </td>
                          <td class="align-middle no-print">
                            <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#editAdminModal" data-id="<?php echo $categorie['id_categorie']; ?>">
                              <span class="fas fa-eye"></span>
                            </button>
                            <a href="mod_categorie.php?id_categorie=<?php echo $categorie["id_categorie"]; ?>" class="btn btn-warning btn-sm mx-3 <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>" data-toggle="tooltip" title="Modifier profil profil">
                              <span class="fas fa-pencil-square" style="color: white;"></span>
                            </a>

                            <button type="button" class="btn btn-danger btn-sm <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>" data-toggle="modal" data-target="#deleteAdminModal" data-id="<?php echo $categorie['id_categorie']; ?>">
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="editAdminModalLabel">Consulter un categorie</h5>
  ...
  <div class="modal fade" id="deleteAdminModal" tabindex="-1" role="dialog" aria-labelledby="deleteAdminModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="deleteAdminModalLabel">Supprimer un categorie</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <p>Êtes-vous sûr de vouloir supprimer cet categorie ?</p>
          <form id="deleteAdminForm" action="<?php $_SERVER["PHP_SELF"]; ?>" method="POST">
            <input type="hidden" id="deleteAdminId" name="categorieId">
  ...
  <script>
    ...
    document.getElementById('exportExcel').addEventListener('click', function(e) {
      ...
      // Sélectionne le tableau des categories (sans les boutons)
