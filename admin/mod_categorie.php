...
if (isset($_GET["id_categorie"]) && !empty($_GET["id_categorie"])) {
    ...
    $requete = $connect->prepare("SELECT * FROM categories WHERE id_categorie=?");
...
if($_SESSION["type_admin"]==2){
  ...
  if (isset($_POST["categorieId"]) && !empty($_POST["categorieId"])) {
    ...
    if (isset($_POST["categorieName"]) && !empty($_POST["categorieName"])) {
        $categorieName = check_input($_POST["categorieName"]);
    } else {
    ...
    if ($mod_ok) {
            $sql = $connect->prepare("UPDATE Categories SET nom_categorie=? WHERE id_categorie=?");
            $requete = $sql->execute(array($categorieName,$id_categorie));
                            ...
                            <div class="card-body px-0 pt-0 pb-2">
                                <form action="<?php echo check_input($_SERVER["PHP_SELF"]); ?>" method="POST" class="mx-3" enctype="multipart/form-data">
                                    <div class="form-group my-1">
                                        <input type="hidden" class="form-control" id="categorieId" name="categorieId" value="<?php if(isset($result["id_categorie"]) && !empty($result["id_categorie"])){ echo $result["id_categorie"];}else if(isset($id_categorie) && !empty($id_categorie)){ echo $id_categorie;} ?>">
                                        <label for="adminName" class="my-0">Nom</label>
                                        <input type="text" class="form-control" id="categorieName" name="categorieName" value="<?php if(isset($result["nom_categorie"]) && !empty($result["nom_categorie"])){ echo $result["nom_categorie"];} else if(isset($categorieName) && !empty($categorieName)){ echo $categorieName;} ?>">
