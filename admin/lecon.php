...
$lecons = $connect->query("SELECT * FROM lecon lec INNER JOIN cours c ON c.id_cours= lec.id_cours")->fetchAll(PDO::FETCH_ASSOC);

if ($_SERVER["REQUEST_METHOD"] == "POST" && $_SESSION["type_admin"] == 1) {
  if (isset($_POST["adminId"]) && !empty($_POST["adminId"])) {
    $id_lecon = check_input($_POST["adminId"]);
    $ok = true;
  } else {
  ...
  if ($ok) {
    $requete = $connect->prepare("DELETE FROM lecon WHERE id_lecon=?");
    $result= $requete->execute(array($id_lecon));
    if($result){
      $_GET["id_lecon"] = $id_lecon;
...
<body class="g-sidenav-show bg-gray-100">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark font-weight-bolder" aria-current="page">lecon</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">Les lecons <?php if (isset($id_lecon)) echo $id_lecon; ?> </h6>
        </nav>
              ...
              <div class="card-body px-0 pt-0 pb-2">
                <div class="table-responsive p-0">
                  <table class="table align-items-center mb-0 table-striped table-bordered table-hover" id="myTable">
                    <thead>
                      <tr class="no-print">
                        <td class="text-sm"><span class="fas fa-list mx-3"></span>Nombre de lécon : <span class="bg-dark p-2 rounded-pill text-white text-b" style="font-weight:bold;"><?php echo count($lecons); ?></span></td>
                        <td class="text-sm" colspan="2">Ajouter une lécon
                          <a href="add_lecon.php" class="btn btn-primary btn-sm mx-3 rounded-pill <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>">
                            <span class="fas fa-plus"></span>
                          </a>
                        </td>
                      </tr>
                      <tr>
                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">lecon <a href="#" id="sortName"><span class="fas fa-sort mx-3" data-toggle="tooltip" data-placement="bottom" title="Trier les élements"></span></a></th>
                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">contenu du cours
                        </th>
                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2 action no-print">date de création</th>

                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2 action no-print">Cours </th>
                      ...
                      <?php foreach ($lecons as $lecon) : ?>
                        <tr class="affiche">
                              ...
                              <div class="d-flex flex-column justify-content-center">
                                <div class="row">
                                  <div class="col-2" style="  align-items:center;">
                                    <span class="bg-dark text-white rounded-pill" style="border-right: 5px solid yellow; padding-right: 10px;"><?= $lecon['id_lecon'] ?></span>
                                  </div>
                                  <div class="col-10">
                                    <h6 class="mb-0 text-sm text-bold"><?= $lecon['titre_lecon'] ?></h6>
                                  </div>
                                </div>
                          ...
                          <td class="text-center">
                            <a href="see_desc_lecon.php?id_lecon=<?= $lecon['id_lecon'] ?>" class="btn btn-primary"><span class="fas fa-eye">voir</span></a>
                          </td>
                          <td>
                          <h6 class="mb-0 text-sm text-bold"><?= $lecon['date_cours'] ?></h6>

                          </td>
                          <td>
                          <h6 class="mb-0 text-sm text-bold"><?= $lecon['nom_cours'] ?></h6>


                          </td>

                          <td class="align-middle no-print">
                            <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#editAdminModal" data-id="<?php echo $lecon['id_lecon']; ?>">
                              <span class="fas fa-eye"></span>
                            </button>
                            <a href="mod_lecon.php?id_lecon=<?php echo $lecon["id_lecon"]; ?>" class="btn btn-warning btn-sm mx-3 <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>" data-toggle="tooltip" title="Modifier profil profil">
                              <span class="fas fa-pencil-square" style="color: white;"></span>
                            </a>

                            <button type="button" class="btn btn-danger btn-sm <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>" data-toggle="modal" data-target="#deleteAdminModal" data-id="<?php echo $lecon['id_lecon']; ?>">
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="editAdminModalLabel">Consulter un lecon</h5>
  ...
  <div class="modal fade" id="deleteAdminModal" tabindex="-1" role="dialog" aria-labelledby="deleteAdminModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="deleteAdminModalLabel">Supprimer un lecon</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <p>Êtes-vous sûr de vouloir supprimer cet lecon ?</p>
  ...
  <script>
    ...
    document.getElementById('exportExcel').addEventListener('click', function(e) {
      ...
      // Sélectionne le tableau des lecon (sans les boutons)
