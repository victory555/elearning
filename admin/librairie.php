...
$livres = $connect->query("SELECT * FROM livres li INNER JOIN categories  cat on cat.id_categorie=li.id_categorie ")->fetchAll(PDO::FETCH_ASSOC);

if ($_SERVER["REQUEST_METHOD"] == "POST" && $_SESSION["type_admin"] == 1) {
  if (isset($_POST["adminId"]) && !empty($_POST["adminId"])) {
    $id_livre = check_input($_POST["adminId"]);
    $ok = true;
  } else {
  ...
  if ($ok) {
    $requete = $connect->prepare("DELETE FROM livres WHERE id_livre=?");
    $requete->execute(array($id_livre));
    $_GET["id_livre"] = $id_livre;
    header("Location:librairie.php");
  }
} else {
  $del_error = 'Seule les super-livres sont autorisés à supprimer <span class="fas fa-circle-exclamation"></span>';
  $ok = false;
}
...
<body class="g-sidenav-show bg-gray-100">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark font-weight-bolder" aria-current="page">livres</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">Les livres <?php if (isset($id_livre)) echo $id_livre; ?> </h6>
        </nav>
              ...
              <div class="card-body px-0 pt-0 pb-2">
                <div class="table-responsive p-0">
                  <table class="table align-items-center mb-0 table-striped table-bordered table-hover" id="myTable">
                    <thead>
                      <tr class="no-print">
                        <td class="text-sm"><span class="fas fa-list mx-3"></span>Nombre d'livres : <span class="bg-dark p-2 rounded-pill text-white text-b" style="font-weight:bold;"><?php echo count($livres); ?></span></td>
                        <td class="text-sm" colspan="2">Ajouter un livre
                          <a href="add_livre.php" class="btn btn-primary btn-sm mx-3 rounded-pill <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>">
                            <span class="fas fa-plus"></span>
                          </a>
                        </td>
                      </tr>
                      <tr>
                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Livres <a href="#" id="sortName"><span class="fas fa-sort mx-3" data-toggle="tooltip" data-placement="bottom" title="Trier les élements"></span></a></th>
                        ...
                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2 action no-print">Lire</th>
                        ...
                        <tr class="affiche">
                          <td>
                            <div class="d-flex px-2 py-1">
                              <div>
                                <img src="images/<?= $livre['photo_livre'] ?>" class="avatar avatar-sm me-3">
                              </div>
                              <div class="d-flex flex-column justify-content-center">
                                <div class="row">
                                  <div class="col-2" style="  align-items:center;">
                                    <span class="bg-dark text-white rounded-pill" style="border-right: 5px solid yellow; padding-right: 10px;"><?= $livre['id_livre'] ?></span>
                                  </div>
                                  <div class="col-10">
                                    <h6 class="mb-0 text-sm text-bold"><?= $livre['nom_livre'] ?></h6>
                                  </div>
                                </div>
                          ...
                          <td>
                            <a href="see_desc.php?id_livre=<?= $livre['id_livre'] ?>" class="btn btn-primary"><span class="fas fa-eye">voir</span></a>
                          </td>
                          <td>
                          <h6 class="mb-0 text-sm text-bold"><?= $livre['nom_categorie'] ?></h6>

                          </td>
                          <td>
                          <a href="see_livre.php?nom_livre=<?= $livre['fichier_livre'] ?>" class="btn btn-dark"><span class="fas fa-eye">voir</span></a>


                          </td>

                          <td class="align-middle no-print">
                            <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#editAdminModal" data-id="<?php echo $livre['id_livre']; ?>">
                              <span class="fas fa-eye"></span>
                            </button>
                            <a href="mod_livre.php?id_livre=<?php echo $livre["id_livre"]; ?>" class="btn btn-warning btn-sm mx-3 <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>" data-toggle="tooltip" title="Modifier profil profil">
                              <span class="fas fa-pencil-square" style="color: white;"></span>
                            </a>

                            <button type="button" class="btn btn-danger btn-sm <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>" data-toggle="modal" data-target="#deleteAdminModal" data-id="<?php echo $livre['id_livre']; ?>">
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="editAdminModalLabel">Consulter un livre</h5>
  ...
  <div class="modal fade" id="deleteAdminModal" tabindex="-1" role="dialog" aria-labelledby="deleteAdminModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="deleteAdminModalLabel">Supprimer un livre</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <p>Êtes-vous sûr de vouloir supprimer cet livre ?</p>
  ...
  <script>
    ...
    document.getElementById('exportExcel').addEventListener('click', function(e) {
      ...
      // Sélectionne le tableau des livres (sans les boutons)
