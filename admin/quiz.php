...
$quizs = $connect->query("SELECT * FROM quiz lec INNER JOIN cours c ON c.id_cours= lec.id_cours")->fetchAll(PDO::FETCH_ASSOC);

if ($_SERVER["REQUEST_METHOD"] == "POST" && $_SESSION["type_admin"] == 1) {
  if (isset($_POST["adminId"]) && !empty($_POST["adminId"])) {
    $id_quiz = check_input($_POST["adminId"]);
    $ok = true;
  } else {
  ...
  if ($ok) {
    $requete = $connect->prepare("DELETE FROM quiz WHERE id_quiz=?");
    $result= $requete->execute(array($id_quiz));
    if($result){
...
<body class="g-sidenav-show bg-gray-100">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark font-weight-bolder" aria-current="page">quiz</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">Les quizs <?php if (isset($id_quiz)) echo $id_quiz; ?> </h6>
        </nav>
              ...
              <div class="card-body px-0 pt-0 pb-2">
                <div class="table-responsive p-0">
                  <table class="table align-items-center mb-0 table-striped table-bordered table-hover" id="myTable">
                    <thead>
                      <tr class="no-print">
                        <td class="text-sm"><span class="fas fa-list mx-3"></span>Nombre de quiz : <span class="bg-dark p-2 rounded-pill text-white text-b" style="font-weight:bold;"><?php echo count($quizs); ?></span></td>
                        <td class="text-sm" colspan="2">Ajouter un quiz
                          <a href="add_quiz.php" class="btn btn-primary btn-sm mx-3 rounded-pill <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>">
                            <span class="fas fa-plus"></span>
                          </a>
                        </td>
                      </tr>
                      <tr>
                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">quiz <a href="#" id="sortName"><span class="fas fa-sort mx-3" data-toggle="tooltip" data-placement="bottom" title="Trier les élements"></span></a></th>
                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">questions
                        </th>
                    ...
                    <tbody>
                      <?php foreach ($quizs as $quiz) : ?>
                        <tr class="affiche">
                              ...
                              <div class="d-flex flex-column justify-content-center">
                                <div class="row">
                                  <div class="col-2" style="  align-items:center;">
                                    <span class="bg-dark text-white rounded-pill" style="border-right: 5px solid yellow; padding-right: 10px;"><?= $quiz['id_quiz'] ?></span>
                                  </div>
                                  <div class="col-10">
                                    <h6 class="mb-0 text-sm text-bold"><?= $quiz['titre_quiz'] ?></h6>
                                  </div>
                                </div>
                          ...
                          <h6 class="mb-0 text-sm text-bold"><?= $quiz['questions'] ?></h6>


                          <td>
                          <h6 class="mb-0 text-sm text-bold"><?= $quiz['nom_cours'] ?></h6>


                          </td>

                          <td class="align-middle no-print">
                            <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#editAdminModal" data-id="<?php echo $quiz['id_quiz']; ?>">
                              <span class="fas fa-eye"></span>
                            </button>
                            <a href="mod_quiz.php?id_quiz=<?php echo $quiz["id_quiz"]; ?>" class="btn btn-warning btn-sm mx-3 <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>" data-toggle="tooltip" title="Modifier profil profil">
                              <span class="fas fa-pencil-square" style="color: white;"></span>
                            </a>

                            <button type="button" class="btn btn-danger btn-sm <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>" data-toggle="modal" data-target="#deleteAdminModal" data-id="<?php echo $quiz['id_quiz']; ?>">
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="editAdminModalLabel">Consulter un quiz</h5>
  ...
  <div class="modal fade" id="deleteAdminModal" tabindex="-1" role="dialog" aria-labelledby="deleteAdminModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="deleteAdminModalLabel">Supprimer un quiz</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <p>Êtes-vous sûr de vouloir supprimer cet quiz ?</p>
  ...
  <script>
    ...
    document.getElementById('exportExcel').addEventListener('click', function(e) {
      ...
      // Sélectionne le tableau des quiz (sans les boutons)
