...
$tags = $connect->query("SELECT * FROM tags")->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_SESSION["type_admin"] == 1) {
  ...
  if ($ok) {
    $requete = $connect->prepare("DELETE FROM tags WHERE id_tag=?");
...
<body class="g-sidenav-show bg-gray-100">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark font-weight-bolder" aria-current="page">tags</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">Les tags <?php if (isset($id_admin)) echo $id_admin; ?> </h6>
        </nav>
              ...
              <div class="card-body px-0 pt-0 pb-2">
                <div class="table-responsive p-0">
                  <table class="table align-items-center mb-0 table-striped table-bordered table-hover" id="myTable">
                    <thead>
                      <tr class="no-print">
                        <td class="text-sm"><span class="fas fa-list mx-3"></span>Nombre de tags : <span class="bg-dark p-2 rounded-pill text-white text-b" style="font-weight:bold;"><?php echo count($tags); ?></span></td>
                        <td class="text-sm" colspan="2">Ajouter un tag
                          <a href="add_tag.php" class="btn btn-primary btn-sm mx-3 rounded-pill <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>">
                            <span class="fas fa-plus"></span>
                          </a>
                        </td>
                      </tr>
                      <tr>
                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Tags <a href="#" id="sortName"><span class="fas fa-sort mx-3" data-toggle="tooltip" data-placement="bottom" title="Trier les élements"></span></a></th>
                          ...
                          <td>
                              <div class="d-flex flex-column justify-content-center">
                                  <div style="align-items:center;">
                                    <span class="btn btn-sm btn-dark rounded-pill my-2" style="border-right: 5px solid yellow; padding-right: 10px;"><?= $tag['id_tag'] ?></span>
                                    <h6 class="mb-0 text-sm text-bold" style="display: inline-block;"><?= $tag['nom_tag'] ?></h6>

                                  </div>                                
                          </td>
                          <td class="align-middle no-print">
                            <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#editAdminModal" data-id="<?php echo $tag['id_tag']; ?>">
                              <span class="fas fa-eye"></span>
                            </button>
                            <a href="mod_tag.php?id_tag=<?php echo $tag["id_tag"]; ?>" class="btn btn-warning btn-sm mx-3 <?php if ($_SESSION["type_admin"] == 2 && $_SESSION['email_admin'] !== $tag['email_admin']) echo "disabled"; ?>" data-toggle="tooltip" title="Modifier un tag">
                              <span class="fas fa-pencil-square" style="color: white;"></span>
                            </a>

                            <button type="button" class="btn btn-danger btn-sm <?php if ($_SESSION["type_admin"] == 2) echo "disabled"; ?>" data-toggle="modal" data-target="#deleteAdminModal" data-id="<?php echo $tag['id_tag']; ?>">
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="editAdminModalLabel">Consulter un tag</h5>
  ...
  <div class="modal fade" id="deleteAdminModal" tabindex="-1" role="dialog" aria-labelledby="deleteAdminModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="deleteAdminModalLabel">Supprimer un tag</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <p>Êtes-vous sûr de vouloir supprimer cet tag ?</p>
          <form id="deleteAdminForm" action="<?php $_SERVER["PHP_SELF"]; ?>" method="POST">
            <input type="hidden" id="deleteAdminId" name="tagId">
  ...
  <script>
    ...
    document.getElementById('exportExcel').addEventListener('click', function(e) {
      ...
      // Sélectionne le tableau des tags (sans les boutons)
