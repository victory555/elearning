...
if($_SESSION["type_admin"]==2){
    ...
    if (isset($_POST["tagName"]) && !empty($_POST["tagName"])) {
        $nom_tag = check_input($_POST["tagName"]);
        $mod_ok=true;
    } else {
    ...
    if ($mod_ok) {
            $sql = $connect->prepare("INSERT INTO tags(nom_tag) VALUES(?)");
            $requete = $sql->execute(array($nom_tag));
            if($requete){
        ...
        <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" data-scroll="true">
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    ...
                    <h6 class="font-weight-bolder mb-0">Les Tags</h6>
                </nav>
                            ...
                            <div class="card-body px-0 pt-0 pb-2">
                                <form action="<?php echo check_input($_SERVER["PHP_SELF"]); ?>" method="POST" class="mx-3" enctype="multipart/form-data">
                                    <div class="form-group my-1">
                                        ...
                                        <input type="text" class="form-control" id="un tagName" name="tagName" value="<?php if(isset($result["nom_tag"]) && !empty($result["nom_tag"])){ echo $result["nom_tag"];} else if(isset($tagName) && !empty($tagName)){ echo $tagName;} ?>">
                                        <p class="text-danger" style="font-size: 10px; font-weight: bold;"><?php if(isset($namError) && !empty($namError)){ echo $namError;} ?></p>

                                    </div> 
                                    <div class="row mt-3">
                                      <div class="col-md-6">
                                        <button type="submit" class="btn btn-primary  form-control  text-white">Ajouter <span class="fas fa-save mx-3"></span></button>
                                      </div>
                                      <div class="col-md-6">
